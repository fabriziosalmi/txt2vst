...
def get_theme(spec: dict) -> dict:
    ...
    if isinstance(theme_raw, str):
        if theme_raw not in THEMES:
            import sys
            print(f"Warning: unknown theme '{theme_raw}', falling back to midnight", file=sys.stderr)
...
def gen_look_and_feel(spec: dict) -> str:
    ...
    L.append("")
    L.append(f"        // Background & text")
    L.append(f"        setColour(juce::ResizableWindow::backgroundColourId, juce::Colour({theme['bg']}));")
    L.append(f"        setColour(juce::Label::textColourId, juce::Colour({theme['text']}));")
    L.append("")
    L.append(f"        // Sliders / knobs")
    L.append(f"        setColour(juce::Slider::rotarySliderFillColourId, juce::Colour({theme['accent']}));")
    L.append(f"        setColour(juce::Slider::rotarySliderOutlineColourId, juce::Colour({theme['border']}));")
    L.append(f"        setColour(juce::Slider::thumbColourId, juce::Colour({theme['accent']}));")
    L.append(f"        setColour(juce::Slider::trackColourId, juce::Colour({theme['surface']}));")
    L.append("")
    L.append(f"        // Buttons")
    L.append(f"        setColour(juce::TextButton::buttonColourId, juce::Colour({theme['surface']}));")
    L.append(f"        setColour(juce::TextButton::buttonOnColourId, juce::Colour({theme['accent']}));")
    L.append(f"        setColour(juce::TextButton::textColourOffId, juce::Colour({theme['text']}));")
    L.append(f"        setColour(juce::TextButton::textColourOnId, juce::Colour({theme['bg']}));")
    L.append("")
    L.append(f"        // Tooltips")
    L.append(f"        setColour(juce::TooltipWindow::backgroundColourId, juce::Colour({theme['header']}));")
    L.append(f"        setColour(juce::TooltipWindow::textColourId, juce::Colour({theme['text']}));")
    L.append("    }")
    L.append("")
    L.append("    // Theme color constants for custom paint()")
