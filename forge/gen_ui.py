...
def gen_editor_cpp(spec: dict) -> str:
    ...
    ui = spec["plugin"].get("ui", [1280, 760])
    L = []
...
def gen_stepgrid_h(spec: dict) -> str:
    ...
    L.append("    static constexpr int LABEL_W = 64;")
    L.append("    static constexpr int STEP_W = 40;")
    L.append("    static constexpr int STEP_H = 36;")
    L.append("    static constexpr int ROW_H = 72;")
    L.append("    static constexpr int PKNOB_SZ = 48;")
    L.append("    static constexpr int MAX_PKNOBS = 6;")
    L.append("    static constexpr int PKNOB_GAP = 4;")
    L.append("    static constexpr int PARAM_AREA_W = MAX_PKNOBS * (PKNOB_SZ + PKNOB_GAP);")
    L.append("    static constexpr int KNOB_W = 44;")
    L.append("    static constexpr int MS_BTN_W = 28;")
    L.append("    static constexpr int BTN_W = 36;")
    L.append("    static constexpr int HEADER_H = 30;")
    L.append("    static constexpr int PAD = 4;")
...
def gen_stepgrid_cpp(spec: dict) -> str:
    ...
    L.append("        g.setFont(10.0f);")
    L.append("        g.setColour(juce::Colour(0xff999999));")
    L.append("        int px = PAD + LABEL_W + VISIBLE_STEPS * STEP_W + PAD;")
    L.append("        for (int k = 0; k < chParams[ch].count; ++k) {")
    L.append("            g.drawText(chParams[ch].entries[k].label,")
    L.append("                       px + k * (PKNOB_SZ + PKNOB_GAP), ry + ROW_H - 16,")
    L.append("                       PKNOB_SZ, 14, juce::Justification::centred);")
    L.append("        }")
    L.append("")
