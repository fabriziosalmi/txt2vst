...
def gen_processor_h(spec: dict) -> str:
    ...
    L.append('#include "Sequencer.h"')
    master = spec["features"].get("mastering")
    if master:
        L.append('#include "fx/MasterChain.h"')
    L.append("")
    ...
    if master:
        L.append("    MasterChain masterChain;")
...
def gen_processor_cpp(spec: dict) -> str:
    ...
    if mastering:
        preset_map = {"transparent": 1, "punch": 2, "wet": 3, "radio": 4, "distorted": 5, "wide": 6}
        pid = preset_map.get(mastering.lower(), 1)
        L.append(f"    masterChain.prepare(sampleRate);")
        L.append(f"    masterChain.setPreset({pid});")
    L.append("}")
    L.append("")
    ...
    L.append("    std::array<float, STACK_MAX> tmpL{}, tmpR{};")
    ...
    L.append("            std::memset(tmpL.data(), 0, static_cast<size_t>(blockSize) * sizeof(float));")
    L.append("            std::memset(tmpR.data(), 0, static_cast<size_t>(blockSize) * sizeof(float));")
    L.append("            voiceBank.renderBus(voice, tmpL.data(), tmpR.data(), blockSize, vol);")
    ...
    if mastering:
        ...
        L.append("    // Master chain")
        L.append("    masterChain.process(mainL, mainR, numSamples);")
    else:
        L.append("        for (int i = 0; i < blockSize; ++i) {")
        L.append("            mainL[offset + i] = fastTanh(mainL[offset + i]);")
        L.append("            mainR[offset + i] = fastTanh(mainR[offset + i]);")
